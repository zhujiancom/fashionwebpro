$(function(){
	$("#reportGrid").flexigrid({
		url:"report_showAll.action",
		searchUrl:'report_doSearch.action',
		dataType:"json",
		showcheckbox: true,
		colModel:[{
			display:'Report Name(EN)',
			name:"reportEname",
			width:250,
			align:'center'
		},{
			display:'Report Name(ZH)',
			name:"reportCname",
			width:250,
			align:'center'
		},{
	...
	function action(com,grid){
		switch(com){
			case 'Add':
				openDialog("Add New PressReport Information",'/'+projectname+'/backend/modules/ReportAdmin/report_add.jsp',800,500);
                break;
			case 'Delete':
				...
				if(confirm("sure delete?")){
					delItem(ids,'/'+projectname+'/backend/report_delete.action','#reportGrid');
				}
				break;
			case 'Edit':
				selected_count = $('.trSelected', grid).length;
				...
				openDialog("Modify Report Information",'/'+projectname+'/backend/report_modifyForward.action?id='+id,800,500);
                break;
			case 'Search':
				break;
		}
