$(function(){
	$("#editorialGrid").flexigrid({
		url:"editorial_showAll.action",
		searchUrl:'editorial_doSearch.action',
		dataType:"json",
		showcheckbox: true,
		colModel:[{
			display:'Editorial Name(ZH)',
			name:"editorialCname",
			width:150,
			align:'center'
		},{
			display:'Editorial Name(EN)',
			name:"editorialEname",
			width:150,
			align:'center'
		},{
	...
	function action(com,grid){
		switch(com){
			case 'Add':
				openDialog("Add New Editorial Image Infomation",'/'+projectname+'/backend/modules/EditorialAdmin/editorial_add.jsp',800,500);
                break;
			case 'Delete':
				...
				if(confirm("sure delete?")){
					delItem(ids,'/'+projectname+'/backend/editorial_delete.action','#editorialGrid');
				}
				break;
			case 'Edit':
				selected_count = $('.trSelected', grid).length;
				...
				openDialog("Modify Editorial Images Information",'/'+projectname+'/backend/editorial_modifyForward.action?id='+id,800,500);
                break;
			case 'Search':
				break;
		}
