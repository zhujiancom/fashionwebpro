		...
		return this.each(function() {
					...
					$input.focus(function() {
						...
						if($input.attr("rex")){
							...
							var value = $input.val();
							if(!re.test(value)){
								$wrapper.after("<span style='color:red'><img src=/"+projectName+"/comm_images/error.png width='16px' height='16px'/>&nbsp;&nbsp;数据格式不正确!</span>");
								$input.attr("result",false);
								return;
							}
						}
						...
						if($input.hasClass("unique")){
							...
							if(action == undefined || action == ""){
								$wrapper.after("<span style='color:red'><img src=/"+projectName+"/comm_images/error.png width='16px' height='16px'/>&nbsp;&nbsp;没有action属性!</span>");
								$input.attr("result",false);
								return;
							}
							var param = $input.val();
							...
							$.ajax({
								...
								success:function(data){
									if(data.msg == "1"){
										$wrapper.after("<span style='color:green'><img src=/"+projectName+"/comm_images/correct.png width='16px' height='16px'/>&nbsp;&nbsp;该系统中没有该值,可以注册!</span>");
									}else if(data.msg == "0"){
										$wrapper.after("<span style='color:red'><img src=/"+projectName+"/comm_images/error.png width='16px' height='16px'/>&nbsp;&nbsp;该系统中已有该值,如果不是修改,请重新注册!</span>");
										$input.attr("result",false);
									}
								}
							});
